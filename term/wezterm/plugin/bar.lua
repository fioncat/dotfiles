...
wezterm.on("format-tab-title", function(tab, tabs, _panes, conf, _hover, _max_width)
	...
	-- user can input title from `window:active_tab():set_title()`
	local usertitle = tabi:get_title()
	if usertitle ~= nil and #usertitle > 0 then
		tabtitle = usertitle
	end
...
wezterm.on("update-status", function(window, _pane)
	...
	local background = palette.ansi[5]
	if window:leader_is_active() then
		background = palette.indexed[16]
	end
